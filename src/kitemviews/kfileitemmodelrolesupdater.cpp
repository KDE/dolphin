...
void KFileItemModelRolesUpdater::updateVisibleIcons()
{
    ...
    for (index = m_firstVisibleIndex; index <= lastVisibleIndex && timer.elapsed() < MaxBlockTimeout; ++index) {
        ...
        if (m_previewShown) {
            ...
            const QImage cached = cachedThumbnailWithFallback(item, cacheSize(m_iconSize), fromRequestedBucket);
            if (!cached.isNull()) {
                SmallHash data = rolesData(item, index);
                ...
                if (fromRequestedBucket && KIO::PreviewJob::cachedThumbnailMatchesFile(cached, item)) {
                    m_finishedItems.insert(item);
                }
                continue;
            }
...
{
    // The thumbnail cache has 128/256/512/1024 pixel buckets; PreviewJob picks one from the
    // request scaled by the device pixel ratio. Request the logical 128 or 256; images get
    // downscaled for the frame anyway, so a larger request buys nothing here.
    return (iconSize.width() > 128) || (iconSize.height() > 128) ? QSize(256, 256) : QSize(128, 128);
}

QImage KFileItemModelRolesUpdater::cachedThumbnailWithFallback(const KFileItem &item, const QSize &requestSize, bool &fromRequestedBucket)
{
    fromRequestedBucket = false;
#if KIO_VERSION >= QT_VERSION_CHECK(6, 30, 0)
    QImage cached = KIO::PreviewJob::cachedThumbnail(item, requestSize, m_devicePixelRatio);
    ...
    if (cached.isNull()) {
        ...
        for (const int bucket : {512, 256, 128}) {
            ...
            cached = KIO::PreviewJob::cachedThumbnail(item, QSize(side, side), m_devicePixelRatio);
            if (!cached.isNull()) {
                break;
            }
        }
...
QPixmap KFileItemModelRolesUpdater::cachedPreviewPixmap(const KFileItem &item, const QSize &iconSize)
{
    ...
    bool fromRequestedBucket;
    const QImage cached = cachedThumbnailWithFallback(item, cacheSize(iconSize), fromRequestedBucket);
