...
QPixmap KStandardItemListWidget::pixmapForIcon(const QString &name, const QHash<Qt::Corner, QString> &overlays, int size, QIcon::Mode mode) const
{
    ...
    const QString key = "KStandardItemListWidget:" % name % ":" % overlays.values().join(QLatin1Char(':')) % ":" % QString::number(size) % "@"
    ...
    if (!QPixmapCache::find(key, &pixmap)) {
        ...
        pixmap = addOverlays(pixmap, overlays, pixmap.size(), dpr, mode);
