...
QPixmap KStandardItemListWidget::pixmapForIcon(const QString &name, const QHash<Qt::Corner, QString> &overlays, const QSize &size, QIcon::Mode mode) const
{
    ...
    if (!QPixmapCache::find(key, &pixmap)) {
        ...
        const auto logicalSize = pixmap.size() / pixmap.devicePixelRatio();
        int width = qMax(size.width() / 2 - logicalSize.width(), logicalSize.width());
        int height = qMax(size.height() / 2 - logicalSize.height(), logicalSize.height());

        pixmap = addOverlays(pixmap, overlays, QSize{width, height}, dpr, mode);
