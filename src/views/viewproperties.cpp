...
std::optional<ViewPropertySettings *> ViewProperties::loadProperties(const QString &folderPath) const
{
    ...
    if (file->exists()) {
        ...
        if (config.hasGroup("Dolphin") || config.hasGroup("Settings")) {
            ...
            config.copyTo(bufferConfig.get());
...
void ViewProperties::save()
{
    ...
    auto buffer = std::make_shared<QBuffer>(new QBuffer());
    if (!buffer->open(QIODevice::ReadWrite)) {
        qCWarning(DolphinDebug) << "Could not create buffer";
    }
    ...
    m_node->config()->copyTo(config.get());
