...
void DolphinItemListView::updateGridSize()
{
    ...
    int iconSize = useGlobalViewProps ? (previewsShown() ? settings.previewSize() : settings.iconSize()) : m_iconSize;
    if (iconSize <= 0) {
        // m_iconSize is only set once setZoomLevel() has run. Before that
        // (construction, readSettings(), a layout change) fall back to the
        // stored per-mode size so the icons never collapse to a zero size.
        iconSize = previewsShown() ? settings.previewSize() : settings.iconSize();
    }
