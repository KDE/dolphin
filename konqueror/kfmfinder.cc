...
KfmFinder::KfmFinder( KfmView *_view ) : QListView( _view )
{
  ...
  connect( &m_bufferTimer, SIGNAL( timeout() ), 
...
void KfmFinder::initConfig()
{
  ...
  m_bgColor           = gui->bgColor();
  m_textColor         = gui->textColor();
  m_linkColor         = gui->linkColor();
  m_vLinkColor        = gui->vLinkColor();
  m_stdFontName       = gui->stdFontName();
  m_fixedFontName     = gui->fixedFontName();
  m_fontSize          = gui->fontSize();

//   m_bgPixmap          = gui->bgPixmap();
...
//     viewport()->setBackgroundMode( NoBackground );
  ...
  m_mouseMode         = gui->mouseMode();

  m_underlineLink     = gui->underlineLink();
  m_changeCursor      = gui->changeCursor();
  m_isShowingDotFiles = gui->isShowingDotFiles();
...
void KfmFinder::setBgColor( const QColor& _color )
{ 
  m_bgColor = _color;
...
void KfmFinder::setTextColor( const QColor& _color )
{
  m_textColor = _color;
...
void KfmFinder::setLinkColor( const QColor& _color )
{
  m_linkColor = _color;
...
void KfmFinder::setVLinkColor( const QColor& _color )
{
  m_vLinkColor = _color;
...
void KfmFinder::setStdFontName( const char *_name )
{
  m_stdFontName = _name;
...
void KfmFinder::setFixedFontName( const char *_name )
{
...
void KfmFinder::setFontSize( const int _size )
{
  m_fontSize = _size;
...
void KfmFinder::setBgPixmap( const QPixmap& _pixmap )
{
  m_bgPixmap = _pixmap;
...
void KfmFinder::setUnderlineLink( bool _underlineLink )
{
...
void KfmFinder::setShowingDotFiles( bool _isShowingDotFiles )
{
...
bool KfmFinder::eventFilter( QObject *o, QEvent *e )
{
  if ( o != viewport() )
     return false;
...
void KfmFinder::dragMoveEvent( QDragMoveEvent *_ev )
{
  KfmFinderItem *item = (KfmFinderItem*)itemAt( _ev->pos() );
  if ( !item )
  {
    if ( m_dragOverItem )
...
void KfmFinder::dragEnterEvent( QDragEnterEvent *_ev )
{
  m_dragOverItem = 0L;
...
void KfmFinder::dragLeaveEvent( QDragLeaveEvent * )
{
  if ( m_dragOverItem != 0L )
    setSelected( m_dragOverItem, false );
  m_dragOverItem = 0L;
...
void KfmFinder::dropEvent( QDropEvent * e )
{
  if ( m_dragOverItem != 0L )
    setSelected( m_dragOverItem, false );
  m_dragOverItem = 0L;
...
void KfmFinder::slotDirectoryDirty( const char *_dir )
{
  string f = _dir;
  K2URL::encode( f );

  QDictIterator<KfmFinderDir> it( m_mapSubDirs );
  for( ; it.current(); ++it )
  {    
...
void KfmFinder::addSubDir( const char* _url, KfmFinderDir* _dir )
{
...
void KfmFinder::removeSubDir( const char *_url )
{
...
void KfmFinder::keyPressEvent( QKeyEvent *_ev )
{
  ...
  KfmFinderItem* item = (KfmFinderItem*)currentItem();
...
void KfmFinder::mousePressEvent( QMouseEvent *_ev )
{
  m_pressed = true;
  m_pressedPos = _ev->pos();
  m_pressedItem = (KfmFinderItem*)itemAt( _ev->pos() );
...
void KfmFinder::mouseReleaseEvent( QMouseEvent *_mouse )
{
  if ( !m_pressed )
    return;
...
void KfmFinder::mouseMoveEvent( QMouseEvent *_mouse )
{
  if ( !m_pressed || !m_pressedItem )
  {
    if ( isSingleClickArea( _mouse->pos() ) )
    {
      KfmFinderItem* item = (KfmFinderItem*)itemAt( _mouse->pos() );
	...
	if ( m_mouseMode == KfmAbstractGui::SingleClick && m_changeCursor )
...
bool KfmFinder::isSingleClickArea( const QPoint& _point )
{
  if ( itemAt( _point ) )
  {
    int x = _point.x();
...
void KfmFinder::slotOnItem( KfmFinderItem* _item)
{
  if ( !_item )
  {
...
void KfmFinder::selectedItems( list<KfmFinderItem*>& _list )
{
  iterator it = begin();
  for( ; it != end(); it++ )
    if ( it->isSelected() )
...
void KfmFinder::slotReturnPressed( QListViewItem *_item )
{
  if ( !_item )
    return;
...
void KfmFinder::slotRightButtonPressed( QListViewItem *_item, const QPoint &_global, int _column )
{
...
void KfmFinder::popupMenu( const QPoint& _global )
{ 
  QStrList urls;
    
  list<KfmFinderItem*> items;
  selectedItems( items );
  mode_t mode = 0;
  bool first = true;
  list<KfmFinderItem*>::iterator itit = items.begin();
...
void KfmFinder::openURL( const char *_url )
{
  ...
  if ( m_iColumns == -1 || isNewProtocol )
  {
    ...
    {
      string tmp = i18n( "Unknown Protocol " );
      tmp += url.protocol();
      QMessageBox::critical( this, i18n( "Error" ), tmp.c_str(), i18n( "OK" ) );
      return;
    }
...
void KfmFinder::slotError( int /*_id*/, int _errid, const char* _errortext )
{
  kioErrorDialog( _errid, _errortext );
}

void KfmFinder::slotCloseURL( int /*_id*/ )
{
...
void KfmFinder::openSubFolder( const char *_url, KfmFinderDir* _dir )
{
  K2URL url( _url );
  if ( url.isMalformed() )
  {
  ...
  /** Debug code **/
  assert( m_iColumns != -1 && !m_strURL.empty() );
  K2URL u( m_strURL );
  if ( strcmp( u.protocol(), url.protocol() ) != 0 )
    assert( 0 );
  /** End Debug code **/
  ...
  KIOJob* job = new KIOJob;
  connect( job, SIGNAL( sigListEntry( int, UDSEntry& ) ), this, SLOT( slotListEntry( int, UDSEntry& ) ) );
  connect( job, SIGNAL( sigFinished( int ) ), this, SLOT( slotCloseSubFolder( int ) ) );
  connect( job, SIGNAL( sigError( int, int, const char* ) ),
	   this, SLOT( slotError( int, int, const char* ) ) );
...
void KfmFinder::slotCloseSubFolder( int /*_id*/ )
{
...
void KfmFinder::slotListEntry( int /*_id*/, UDSEntry& _entry )
{
...
void KfmFinder::slotBufferTimeout()
{
...
KfmFinder::iterator& KfmFinder::iterator::operator++()
{
  if ( !m_p ) return *this;
  KfmFinderItem *i = (KfmFinderItem*)m_p->firstChild();
  if ( i )
  {
    m_p = i;
    return *this;
  }
  i = (KfmFinderItem*)m_p->nextSibling();
  if ( i )
  {
    m_p = i;
    return *this;
  }
  m_p = (KfmFinderItem*)m_p->parent();
  if ( m_p )
    m_p = (KfmFinderItem*)m_p->nextSibling();
  
  return *this;
}

KfmFinder::iterator KfmFinder::iterator::operator++(int)
{
  KfmFinder::iterator it = *this;
  if ( !m_p ) return it;
  KfmFinderItem *i = (KfmFinderItem*)m_p->firstChild();
  if ( i )
  {
...
void KfmFinder::updateDirectory()
{
  if ( !m_bTopLevelComplete || !m_bSubFolderComplete )
    return;
  
  updateDirectory( 0L, m_strURL.c_str() );
}

void KfmFinder::updateDirectory( KfmFinderDir *_dir, const char *_url )
{
  // Stop running jobs
  if ( m_jobId )
  {
  ...
  KIOJob* job = new KIOJob;
  connect( job, SIGNAL( sigListEntry( int, UDSEntry& ) ), this,
...
void KfmFinder::slotUpdateError( int /*_id*/, int _errid, const char *_errortext )
{
  kioErrorDialog( _errid, _errortext );
...
void KfmFinder::slotUpdateFinished( int /*_id*/ )
{
  if ( m_bufferTimer.isActive() )
  {    
    m_bufferTimer.stop();
  ...
  while( item )
  {
    if ( !((KfmFinderItem*)item)->isMarked() )
    {
      cerr << "Removing " << ((KfmFinderItem*)item)->name() << endl;
      lst.append( item );
    }
    item = item->nextSibling();
  }
...
void KfmFinder::slotUpdateListEntry( int /*_id*/, UDSEntry& _entry )
{
  m_buffer.push_back( _entry );
}

void KfmFinder::drawContentsOffset( QPainter* _painter, int _offsetx, int _offsety,
...
KfmFinderItem::KfmFinderItem( KfmFinder *_finder, KfmFinderDir *_parent, UDSEntry& _entry, K2URL& _url )
  : QListViewItem( _parent )
{
  init( _finder, _entry, _url );
}

KfmFinderItem::KfmFinderItem( KfmFinder *_parent, UDSEntry& _entry, K2URL& _url )
  : QListViewItem( _parent )
{
  init( _parent, _entry, _url );
}

void KfmFinderItem::init( KfmFinder* _finder, UDSEntry& _entry, K2URL& _url )
{
  m_pFinder = _finder;
  m_entry = _entry;
  m_strURL = _url.url();
...
bool KfmFinderItem::acceptsDrops( QStrList& /* _formats */ )
{
...
void KfmFinderItem::returnPressed()
{
  mode_t mode = 0;
...
/*
void KfmFinderItem::popupMenu( const QPoint &_global, int _column )
{
  mode_t mode = 0;
  UDSEntry::iterator it = m_entry.begin();
  for( ; it != m_entry.end(); it++ )
...
const char* KfmFinderItem::key( int _column, bool ) const
{
...
const char* KfmFinderItem::text( int _column ) const
{
  //  assert( _column < (int)m_entry.size() );
...
const char* KfmFinderItem::makeNumericString( const UDSAtom &_atom ) const
{
...
const char* KfmFinderItem::makeTimeString( const UDSAtom &_atom ) const
{
  static char buffer[ 100 ];
...
const char* KfmFinderItem::makeTypeString( const UDSAtom &_atom ) const
{
...
const char* KfmFinderItem::makeAccessString( const UDSAtom &_atom ) const
{
  static char buffer[ 12 ];
...
void KfmFinderItem::paintCell( QPainter *_painter, const QColorGroup & cg, int column, int width, int alignment )  
{
  if ( m_pFinder->m_mouseMode == KfmAbstractGui::SingleClick && 
       m_pFinder->m_underlineLink && column == 0)
  {
...
KfmFinderDir::KfmFinderDir( KfmFinder *_parent, UDSEntry& _entry, K2URL& _url )
  : KfmFinderItem( _parent, _entry, _url )
{
  string url = _url.url();
...
KfmFinderDir::KfmFinderDir( KfmFinder *_finder, KfmFinderDir * _parent, UDSEntry& _entry, K2URL& _url )
  : KfmFinderItem( _finder, _parent, _entry, _url )
{
  string url = _url.url();
  m_pFinder->addSubDir( url.c_str(), this );
...
KfmFinderDir::~KfmFinderDir()
{
  if ( m_pFinder )
    m_pFinder->removeSubDir( m_strURL.c_str() );
}

void KfmFinderDir::setup()
{
  setExpandable( true );
  QListViewItem::setup();
}

void KfmFinderDir::setOpen( bool _open )
{
...
string KfmFinderDir::url( int _trailing )
{
  string tmp;
  K2URL u( m_strURL );
  tmp = u.url( _trailing );
